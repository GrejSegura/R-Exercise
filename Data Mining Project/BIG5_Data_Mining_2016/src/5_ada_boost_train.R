...
dtaMining <- fread("./dta/dtaminingClean_for_logisticRegression.csv")
...
train_dta$label <- as.factor(train_dta$label)
test_dta$label <- as.factor(test_dta$label)
...
adaboostModel <- ada(
	label ~.,
	data = train_dta,
	control = rpart.control(maxdepth = 30, cp = 0.010000, minsplit = 20, xval = 10), iter = 100
)
